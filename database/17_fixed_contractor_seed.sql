...
DELETE FROM portfolio_projects WHERE company_id IN (SELECT id FROM companies WHERE type = 'contractor');
DELETE FROM certificates WHERE company_id IN (SELECT id FROM companies WHERE type = 'contractor');
DELETE FROM company_reviews WHERE company_id IN (SELECT id FROM companies WHERE type = 'contractor');
DELETE FROM company_ratings WHERE company_id IN (SELECT id FROM companies WHERE type = 'contractor');
DELETE FROM companies WHERE type = 'contractor';

-- Step 3: Ensure we have job categories (skip if they already exist)
...
-- Step 4: Insert contractor companies
...
-- Step 5: Insert company ratings for contractors
...
-- Step 6: Insert sample reviews for contractors
...
-- Step 7: Insert certificates for contractors
...
-- Step 8: Insert portfolio projects for contractors
...
-- Step 9: Re-enable RLS policies (optional - you can skip this if you want to keep RLS disabled)
